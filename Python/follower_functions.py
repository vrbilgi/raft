...
def acceptor(server, data, addr):
	...
	_type = Msg.type
	print("I am thread =",str(threading.get_ident()))
	print(str(threading.get_ident()),"Server =",str(server))
	print(str(threading.get_ident()),"addr =",str(addr))
	print(str(threading.get_ident()),"Inside acceptor message type is ",str(_type))
	...
	if _type == 'change':
		...
		if server.role != 'leader':
			...
			s.sendto(pickle.dumps(redirect_msg), ("localhost",server.addressbook[redirect_target]))
			s.close()			
	...
	if _type == 'client' or _type == 'redirect':
		...
		if msg_string == 'show':
			...
			print(str(threading.get_ident()),"msg_string",msg_string)
			print(str(threading.get_ident()),"server=",str(server.role))
			ticket_num = int(msg_string.split()[1])
			if server.role == 'leader':
				print(f"I am the leader, customer wants to buy {ticket_num} tickets")
			...
			# we need to redirect the request to leader
				...
				print(str(threading.get_ident()),'redirect the request to leader')
				if server.leaderID != 0:
					print(str(threading.get_ident()),'if')
					redirect_target = server.leaderID
				else:
					print(str(threading.get_ident()),'else->',str(server.peers))
					redirect_target = random.choice(server.peers)
				redirect_msg = RequestRedirect(msg_string, Msg.uuid, addr)
				try:
					s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
					print(str(threading.get_ident()),"rediredt -port",server.addressbook[redirect_target])
					s.sendto(pickle.dumps(redirect_msg), ("localhost",server.addressbook[redirect_target]))
					s.close()
				except Exception as e:
					print(str(threading.get_ident()),'XXXXXXXXXXX')
		print(str(threading.get_ident()),'Returning from method')
		return
	...
	if _type == 1: # requestvote message
		if _sender not in server.peers:
			return
		_msg = Msg.data
		print(str(threading.get_ident()),'---------Get requestvote message---------')
		_msg = _msg.split()
		...
		if _term < server.currentTerm:
			...
			print('find higher term in RequestVoteMsg')
		...
		s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
		print(str(threading.get_ident()),"replying to port ",str(server.addressbook[_sender]))
		print(str(threading.get_ident()),"My message ",str(reply))
		s.sendto(pickle.dumps(reply_msg), ("localhost",server.addressbook[_sender]))

	elif _type == 2: # Vote response message
		_msg = Msg.data
		print(str(threading.get_ident()),'---------Get vote response message---------')
		...
		if voteGranted:
			if server.during_change == 0:
				if server.role == 'candidate':
					...
					# becomes a leader
						server.role = 'leader'
						if server.follower_state.is_alive():
				...
				if server.oldVotes >= majority_1 and server.newVotes >= majority_2:
					print(str(threading.get_ident()),'Get majority from both old and new, become leader at Term {server.currentTerm}')
					if server.election.is_alive():
						server.election.kill()
					server.role = 'leader'
					if server.follower_state.is_alive():
				...
				if server.newVotes >= majority:
					print('Get majority from new, become leader at Term {server.currentTerm}')
					if server.election.is_alive():
						server.election.kill()
					server.role = 'leader'
					if server.follower_state.is_alive():
